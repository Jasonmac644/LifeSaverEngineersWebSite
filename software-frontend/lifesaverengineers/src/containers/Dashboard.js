...
const Dashboard = () => {
  ...
  function AnyReactComponent({ text, style }) {
  ...
  const fetchdata = async () => {
    ...
    try {
      ...
      docSnap.forEach((doc) => {
        doc.data().vitalsData != undefined && results.push(dayjs(doc.id)); // Assuming doc.id is a date string
      });
    } catch (error) {
  ...
  const points = [{ id: 1, title: "Humber", lat: 43.724, lng: -79.605 }];
  return (
        ...
        <div style={{ height: "350px", width: "90%", borderRadius: "20px" }}>
          ...
          >
            {points.map(({ lat, lng, id, title }) => {
              return (
                <AnyReactComponent key={id} lat={lat} lng={lng} text={id} />
              );
            })}
          </GoogleMapReact>
        </div>
